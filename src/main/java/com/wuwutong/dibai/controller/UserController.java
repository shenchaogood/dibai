...
import com.wuwutong.dibai.vo.IdentityForm;
...
public class UserController {
	...
	public ResponseResult<Long> sendIdentifyCode(String mobile) throws IOException{
		String code = String.format("%04d", random.nextInt(9999));
		return smsBiz.sendIdentifyCode(mobile,code);
	}
	
	@RequestMapping("/register")
	public ResponseResult<User> register(LoginForm loginForm){
		...
		ResponseResult<User> ret=userBiz.register(loginForm.getMobile(),loginForm.getIdentifyCode(),loginForm.getInvitationCode(),loginForm.getIdentifyCodeId());
	...
	public ResponseResult<User> payDeposit(){
		User user=httpSession.getAttibute(Constants.CURRENT_LOGIN_USER, User.class);
		if(user==null){
			return ResponseResult.createFail("请先完成手机验证");
		}
		if(user.getAuthStatus()!=AuthStatus.PAY_DEPOSIT){
			return ResponseResult.createFail("已缴纳押金，无需重复缴纳");
		}
		ResponseResult<User> ret=userBiz.payDeposit(user);
		return ret;
	}
	
	@RequestMapping("/auth")
	public ResponseResult<User> auth(IdentityForm form){
